...
float4 mod289_c4D(float4 x)
{
	return x - floor(x * (1.0 / 289.0)) * 289.0;
}

float4 permute_c4D(float4 x)
{
	return mod289_c4D(((x * 34.0) + 1.0) * x);
}

float4 taylorInvSqrt_c4D(float4 r)
{
	return 1.79284291400159 - 0.85373472095314 * r;
}

float4 fade_c4D(float4 t) {
...
float cnoise(float4 P)
{
	...
	float4 ixy = permute_c4D(permute_c4D(ix) + iy);
	float4 ixy0 = permute_c4D(ixy + iz0);
	float4 ixy1 = permute_c4D(ixy + iz1);
	float4 ixy00 = permute_c4D(ixy0 + iw0);
	float4 ixy01 = permute_c4D(ixy0 + iw1);
	float4 ixy10 = permute_c4D(ixy1 + iw0);
	float4 ixy11 = permute_c4D(ixy1 + iw1);
	...
	float4 norm00 = taylorInvSqrt_c4D(float4(dot(g0000, g0000), dot(g0100, g0100), dot(g1000, g1000), dot(g1100, g1100)));
	g0000 *= norm00.x;
	g0100 *= norm00.y;
	g1000 *= norm00.z;
	g1100 *= norm00.w;

	float4 norm01 = taylorInvSqrt_c4D(float4(dot(g0001, g0001), dot(g0101, g0101), dot(g1001, g1001), dot(g1101, g1101)));
	g0001 *= norm01.x;
	g0101 *= norm01.y;
	g1001 *= norm01.z;
	g1101 *= norm01.w;

	float4 norm10 = taylorInvSqrt_c4D(float4(dot(g0010, g0010), dot(g0110, g0110), dot(g1010, g1010), dot(g1110, g1110)));
	g0010 *= norm10.x;
	g0110 *= norm10.y;
	g1010 *= norm10.z;
	g1110 *= norm10.w;

	float4 norm11 = taylorInvSqrt_c4D(float4(dot(g0011, g0011), dot(g0111, g0111), dot(g1011, g1011), dot(g1111, g1111)));
	g0011 *= norm11.x;
	g0111 *= norm11.y;
	g1011 *= norm11.z;
	g1111 *= norm11.w;
