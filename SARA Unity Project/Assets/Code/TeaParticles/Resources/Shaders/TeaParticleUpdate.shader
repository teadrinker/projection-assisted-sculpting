...
// .w   = particle life (normalized)
//
Shader "teadrinker/TeaParticleUpdate"
{
    Properties
    {
        _MainTex     ("-", 2D)     = ""{}
    ...
    float4 new_particle(float2 uv)
    {
        ...
        float4 offs = float4(1e8, 1e8, 1e8, -1e8) * (uv.x*0.99 + uv.y*(1./256.) > _Config.x*_Config.x);
...
#ifdef NOISE_ON
        // Add noise vector.
        p = (p + _NoiseTime * _NoiseParams.z) * _NoiseParams.x;
    ...
    float nonlinearSpread(float x, float a) {
        float c = x * x;
        c *= c;
        c *= c;
        c *= c;
    ...
    float4 frag_update(v2f_img i) : SV_Target 
    {
        ...
        if (p.w > 0)
        {
            ...
            //p.w -= dt;                               // life
            float lifeDuration = _Config.y * lerp(1., nonlinearSpread(nrand(i.uv, 0), 2.), _LifeVariance);
            //float lifeDuration = _Config.y * (0.5 + nrand(i.uv, 0));
            p.w -= dt / lifeDuration; 
            return p;
        }
        else
